...
| --- | --- | --- |
| 分割模型权重未随代码提交 | `WEIGHTS_AND_DATA.md` | 在受控位置提供 PPGL 分割权重与 TotalSegmentator 权重，并配置对应路径。 |
| TotalSegmentator 运行资产缺失 | 全器官分割依赖 TotalSegmentator 命令及预训练权重 | 安装 TotalSegmentator 并配置权重目录和命令可用性。 |
...
| PPGL 分割权重不随仓库提交 | 默认从 `ai-backend/progress_patch_v5/weights/model_best.pth` 加载 | 在部署环境放置已授权权重，或使用 `PPGL_V5_CHECKPOINT` 覆盖相对路径。 |
