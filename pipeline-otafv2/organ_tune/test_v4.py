...
from Dataloader_mutil import (
    build_image_only_val_transforms,
    build_val_transforms,
    parse_data_list,
    parse_image_only_data_list,
)
...
PARTIAL_EXPORT_CHANNEL_TO_LABEL = {
    0: 1,
    1: 2,
    2: 3,
    3: 4,
...
def compute_surface_metrics_np(
    ...
    spacing = tuple(float(x) for x in np.asarray(spacing_mm, dtype=np.float64).reshape(-1)[:3])
    pred_surface = get_surface_mask_np(pred)
    gt_surface = get_surface_mask_np(gt)
    pred_points = np.argwhere(pred_surface).astype(np.float64, copy=False) * np.asarray(spacing).reshape(1, 3)
    gt_points = np.argwhere(gt_surface).astype(np.float64, copy=False) * np.asarray(spacing).reshape(1, 3)
...
def extract_spacing_mm_from_batch(batch) -> tuple[float, float, float]:
    affine = None
    ...
    if spacing.shape != (3,) or not np.all(np.isfinite(spacing)) or np.any(spacing <= 0):
        return (1.0, 1.0, 1.0)
    return tuple(float(x) for x in spacing.tolist())
...
def partial_probs_to_labelmap(probs: torch.Tensor, threshold: float) -> torch.Tensor:
    if probs.ndim != 5 or int(probs.shape[0]) != 1:
        raise ValueError(f"partial probs 必须为 [1,C,D,H,W]，got={tuple(probs.shape)}")
    num_channels = int(probs.shape[1])
    export_pairs = [
        (ch_idx, label_id)
        for ch_idx, label_id in PARTIAL_EXPORT_CHANNEL_TO_LABEL.items()
        if ch_idx < num_channels
    ]
    if not export_pairs:
        raise ValueError("没有可导出的 OrganTune 通道")

    channel_indices = [ch_idx for ch_idx, _ in export_pairs]
    label_ids = torch.as_tensor(
        [label_id for _, label_id in export_pairs],
        dtype=torch.long,
        device=probs.device,
    )
    selected = probs[0, channel_indices]
    max_probs, max_indices = torch.max(selected, dim=0)
    labelmap = torch.zeros_like(max_indices, dtype=torch.long)
    mask = max_probs >= float(threshold)
    labelmap[mask] = label_ids[max_indices[mask]]
    return labelmap.unsqueeze(0)
...
def build_args():
    ...
    parser.add_argument(
        "--export-only",
        action="store_true",
        help="Only export partial-label NIfTI predictions; skip metrics and keep stitched output on CPU.",
    )
    ...
    if args.export_only and not args.sigmoid_multilabel:
        raise ValueError("--export-only 当前只支持 --sigmoid-multilabel partial-label 推理")
    if args.export_only and not args.save_nifti_labels:
        raise ValueError("--export-only 必须同时使用 --save-nifti-labels")
    if args.export_only and int(args.overlay_max_cases) > 0:
        raise ValueError("--export-only 不支持 overlay 导出，请设置 --overlay-max-cases 0")
...
def run_partial_test(args):
    ...
    if args.export_only:
        test_files = parse_image_only_data_list(
            split_txt,
            args.data_root,
    ...
    if args.export_only:
        transforms = build_image_only_val_transforms(
        ...
        for batch in progress:
            ...
            try:
                ...
                if args.export_only:
                    ...
                    spacing_mm = extract_spacing_mm_from_batch(batch)
                ...
                logits = sliding_window_inference(
                    ...
                    device=torch.device("cpu") if args.export_only else runtime_device,
                )
                probs = torch.sigmoid(logits)
                del logits
                pred_mask = None if args.export_only else (probs >= args.prob_threshold).float()
                ...
                if not args.export_only:
                    for organ_idx, organ_name in enumerate(organ_names):
                        ...
                        surface_metrics = compute_surface_metrics_np(
                            pred_mask[0, organ_idx].detach().cpu().numpy(),
                            labels[0, organ_idx].detach().cpu().numpy(),
                        ...
                        case_row[f"nsd_1mm_{organ_name}"] = metrics["nsd_1mm"]
                        case_row[f"hd95_mm_{organ_name}"] = metrics["hd95_mm"]
                ...
                if args.save_nifti_labels:
                    pred_labelmap = partial_probs_to_labelmap(probs, threshold=args.prob_threshold)
                    visible_pred = bool(torch.any(pred_labelmap > 0).item())
                    saved_label_path = save_prediction_label_nifti(
                        batch=batch,
                        pred_cls=pred_labelmap,
                ...
                if (not args.export_only) and case_id in overlay_case_ids and len(overlay_paths) < int(args.overlay_max_cases):
    ...
    for organ_name in organ_names:
        case_fields.extend(
            [
                ...
                f"f1_{organ_name}",
                f"nsd_1mm_{organ_name}",
                f"hd95_mm_{organ_name}",
        ...
        class_rows.append(
            {
                ...
                "mean_nsd_1mm_valid_cases": safe_mean(
                    [r["nsd_1mm"] for r in records if np.isfinite(r["nsd_1mm"])]
                ),
                "mean_hd95_mm_valid_cases": safe_mean(
                    [r["hd95_mm"] for r in records if np.isfinite(r["hd95_mm"])]
                ),
    ...
    class_fields = [
        ...
        "mean_f1_valid_cases",
        "mean_nsd_1mm_valid_cases",
        "mean_hd95_mm_valid_cases",
    ...
    summary = {
        ...
        "exported_label_ids": sorted(set(PARTIAL_EXPORT_CHANNEL_TO_LABEL.values())),
    }
