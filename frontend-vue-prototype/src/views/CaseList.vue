  ...
  <div class="page-shell case-list-page">
    <div class="page-header">
      ...
      <div class="page-header-actions">
        ...
        <el-button :icon="UploadFilled" @click="goUpload('ct')">上传 CT</el-button>
        <el-button type="primary" :icon="UploadFilled" @click="goUpload('mri')">上传 MRI</el-button>
      </div>
    </div>
          ...
          <template #default="{ row }">
            <div class="action-buttons">
              <template v-if="row.imaging_type === 'ct'">
                ...
                <el-button size="small" :icon="Edit" :disabled="isCaseActive(row)" @click="renameCtCase(row)">
                  修改编号
                </el-button>
                <el-button
                  v-if="canStartCtTask(row.organ_status?.status)"
                  size="small"
                  :icon="VideoPlay"
                ...
                <el-button
                  v-if="canStartCtTask(row.ppgl_status?.status)"
                  size="small"
                  type="warning"
                  :icon="VideoPlay"
              ...
              <template v-else>
                ...
                <el-button
                  v-if="isGliomaInferenceActive(row.status)"
                  size="small"
                  type="warning"
                  plain
...
async function renameCtCase(row) {
  try {
    ...
    if (!newCaseId) return ElMessage.warning('病例编号不能为空')
...
async function removeCase(row) {
  ...
  try {
    await ElMessageBox.confirm(
      `删除 ${typeName} 病例“${row.case_id}”会同时删除原始影像和所有分析结果，且无法恢复。`,
      '确认删除病例',
